...
decoding the byte string using C<utf8::decode>. C<$options> is a bit field
containing the parser and render options. It defaults to C<OPT_DEFAULT>.
...
=head2 parse
    ...
    my $doc = CommonMark->parse(
        string        => $string,
        normalize     => $normalize,      # Optional
        smart         => $smart,          # Optional
        validate_utf8 => $validate_utf8,  # Optional
    );
...
C<parse_document>. When given a file, calls C<parse_file>. C<normalize>,
...
options. It defaults to C<OPT_DEFAULT>.
...
containing the parser options. It defaults to C<OPT_DEFAULT>.
...
typographic quotation marks, double hyphens into en dashes, and triple
hyphens into em dashes.
